...
- `-u`:	The flag that tells Beeline: "Connect using the following URL."
...
```shell
!sh hdfs dfs -ls /user/hive/warehouse/yearbuckets
```

In Beeline, HDFS commands and Linux shell commands can all be run via the shell escape `!sh`.
Note: When you use `!sh`, Beeline does not understand wildcards (`*`) or pipes (`|`).  Instead of interpreting them as commands, it treats them as literal characters.
...
Run tge HDFS command below to verify that the underlying directory and files were created correctly in HDFS, 
 

```shell
!sh hdfs dfs -ls /tmp/pairs
```
...
## Step 8: Exit the Beeline CLI

Once you are done with your queries, you can exit the Beeline interface and return to the Linux shell by typing:
...
```
 ...
 Alternatively, you can also use `!q`.
...
**Apache Tez** is a high-performance, DAG-based execution engine that is often used as a drop-in replacement for MapReduce.
...
```shell
beeline -u jdbc:hive2://localhost:10000 -hiveconf hive.execution.engine=tez
```
